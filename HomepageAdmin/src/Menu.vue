...
<template>
  <div class="flex flex-column border-right-2 border-transparent w-20rem md:text-sm">
    ...
    <div class="flex flex-column gap-5 pl-3">
      <div v-for="( menu,index ) in itemMenu" :key="index" class="flex flex-column gap-3" @click="handleDivClick(index)">
        <!-- item menu -->
        <div class="flex justify-content-between px-3 align-items-center cursor-pointer h-3rem hover:surface-300 hover:text-blue-600">
          <div class="flex gap-3 align-items-center font-semibold" style="font-size: 0.8rem">
        ...
        <div class="flex flex-column">
          <div v-for="(subMenu, index) in menu.item" :key="index" class="hover:surface-300 py-2 cursor-pointer hover:text-blue-600">
            <span class="w-fit-content pl-6" style="font-size: 0.8rem">{{subMenu.subItem}}</span>
          </div>
        </div>
      </div>
    </div>
